...
Vk.prototype.constructResult = function (authInfo, options, cb) {
			...
			if ('city' in result) {
				request({
					...
					if (err) {
						...
						result.city = response.length === 0 ? '' : response[0].name;
						cb(null, result);
					}
				});
			}
			...
			if ('country' in result) {
				request({
					...
					if (err) {
						...
						var response = JSON.parse(body).response;
						result.country = response.length === 0 ? '' : response[0].name;
						cb(null, result);
					}
				});
			}
	...
	function (err, result) {
		if (err) {
			...
			if (result.bdate) {
				var bd = result.bdate.split('.');
				result.birthday = [bd[2], bd[1], bd[0]].join('-');
			}
			...
			cb(null, {
				...
				data: self.removeWasteFields(result, self.renameRule, savedFields)
			});
		}
	});
