...
function createRedirectUrl (base, token, redirectUrl, verifier) {
	var args = {
		oauth_token: token,
...
OAuth1.prototype.exchangeAuthGrant = function (resultInfo, options, cb) {
	...
	var authHeader = this.generateAuthorizationHeader(this.providerInfo.accessUrl, params, resultInfo.token);
	console.log(authHeader);
	var self = this;
