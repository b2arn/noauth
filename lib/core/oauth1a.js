...
OAuth1.prototype.createAuthRequestUrl = function (options, cb) {
	...
	request({
		...
		if (err) {
			...
			console.log('result.oauth_token_secret', result.oauth_token_secret);
...
OAuth1.prototype.exchangeAuthGrant = function (resultInfo, options, cb) {
	...
	this.storage.take(resultInfo.token, function (err, result) {
		if (err) {
			...
			var authHeader = this.generateAuthorizationHeader(this.providerInfo.accessUrl, params, resultInfo.token, result);

			request({
				url: url.resolve(this.providerInfo.accessUrl, url.format({query: {api_key: resultInfo.token}})),
				body: null,
				headers: {Authorization: authHeader},
				method: 'POST'
			}, function (err, res, body) {
