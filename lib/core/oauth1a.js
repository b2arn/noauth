...
OAuth1.prototype.exchangeAuthGrant = function (resultInfo, options, cb) {
	...
	this.storage.take(resultInfo.token, function (err, result) {
		if (err) {
			...
			var authHeader = self.generateAuthorizationHeader(self.providerInfo.accessUrl, params, resultInfo.token, result);

			request({
				url: url.resolve(self.providerInfo.accessUrl, url.format({query: {api_key: resultInfo.token}})),
				body: null,
				headers: {Authorization: authHeader},
				method: 'POST'
			}, function (err, res, body) {
