...
Auth.prototype.checkBodyIsObject = function (statusCode, parsedBody) {
	if (parsedBody.constructor !== Object && parsedBody.constructor !== Array) {
...
Auth.prototype.handleError = function (res, body, cb) {
	var parsedBody = this.parse(body);
	var err =
		this.checkStatusCode(res.statusCode, body) ||
		this.checkBodyIsObject(res.statusCode, parsedBody) ||
		this.parseError(res.statusCode, parsedBody) ||
		(res.statusCode >= 400 ? new NoError('ProviderError', 'Unknown error with body: ' + parsedBody, body) : null);
