...
var Auth = function (opt_appInfo, opt_redirectUrl) {
	...
	var self = this;
	this.boundParseError = function (statusCode, body, cb) {
		self.parseError(statusCode, body, cb);
	};
};
...
Auth.prototype.parseRedirectError = function (query, cb) {
	cb(null, query);
};
...
Auth.prototype.getErrorCode = function (error) {
	return error.name;
};
...
Auth.prototype.genFields = function (options, cb) {
	...
	if (options && options.fields) {
		for (var i = 0; i < options.fields.length; i++) {
			...
			if (key in result) {
				err = new NoInputError(key, 'duplicate');
			}
			result[key] = null;
		}
...
Auth.prototype.checkField = function (field) {
	...
	if (!isKnownField) {
		result = new NoInputError(field, 'wrong field');
	}
	return result;
};
...
Auth.prototype.handleError = function (statusCode, body, cb) {
	...
	var err =
		...
		(statusCode >= 400 && new NoError('ProviderError', 'Unknown error with body: ' + parsedBody, body));
